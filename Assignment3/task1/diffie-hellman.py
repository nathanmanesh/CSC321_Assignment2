...
def diffie_hellman_exchange(q: int, a: int):
    ...
    # alice and both compute shared secret using others public value
    sA = pow(yB, xA, q)
    sB = pow(yA, xB, q)

    assert sA == sB
    
    # return secrets, public values, and share secret

    return (xA, yA), (xB, yB), sA
...
def demo(q: int, a: int):
    ...
    ciphertext_alice = aes_cbc_encrypt(key, iv, message_alice)
    decrypted_by_bob = aes_cbc_decrypt(key, iv, ciphertext_alice)


    message_bob = b"Hi Alice, my name is Bob!"
    ciphertext_bob = aes_cbc_encrypt(key, iv, message_bob)
    decrypted_by_alice = aes_cbc_decrypt(key, iv, ciphertext_bob)


    print("Alice secret:", xA)
    print("Bob secret:", xB)
    print("Alice public:", yA)
    print("Bob public:", yB)
    print("Shared secret:", shared_secret)
    print("AES key:", key.hex())
    print("Bob decrypts:", decrypted_by_bob)
    print("Alice decrypts:", decrypted_by_alice)
